...
import { View, TextInput, ActivityIndicator, Alert, Pressable } from 'react-native';
import {
    makeContractCall,
...
import { useWalletPrivateData, useWalletData } from '../../../../shared/hooks/useWalletData';
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                const poolData = data.find((p: any) => p.pool_id === `${CORE_CONTRACT_ADDRESS}.${POOL_CONTRACT_NAME}`);
        ...
        const lpToken = tokens.find(t => t.key === `${CORE_CONTRACT_ADDRESS}.${POOL_CONTRACT_NAME}::pool-token` || t.symbol === 'STX-stSTX-LP');
        ...
        <View className="bg-custom_complement p-4 rounded-xl border border-custom_border mt-4">
            <View className="mb-4 flex-row justify-between items-start">
                <View>
                    ...
                    <TextWithFont customStyle="text-custom_accent text-lg font-bold">Bitflow STX-stSTX</TextWithFont>
                </View>
            ...
            <View className="flex-row mt-4 bg-custom_background rounded-lg border border-custom_border overflow-hidden">
                ...
                >
                    <TextWithFont customStyle={`${!isWithdrawMode ? 'text-black font-bold' : 'text-white'}`}>Deposit</TextWithFont>
                </Pressable>
                <Pressable
                    onPress={() => setIsWithdrawMode(true)}
                    className={`flex-1 py-2 items-center ${isWithdrawMode ? 'bg-custom_accent' : ''}`}
                >
                    <TextWithFont customStyle={`${isWithdrawMode ? 'text-black font-bold' : 'text-white'}`}>Withdraw</TextWithFont>
                </Pressable>
            </View>
                ...
                <View className="space-y-4 mt-4">
                    <View>
                        ...
                        <View className="flex-row items-center space-x-2">
                            ...
                            />
                            <Button text="MAX" onPress={() => handleStxAmountChange(stxBalance)} customStyle="w-12 h-12" iconName="PlusCircle" />
                        </View>
                    </View>
                    ...
                    <View className="mt-4">
                        ...
                        <View className="flex-row items-center space-x-2">
                            ...
                            />
                            <Button text="MAX" onPress={() => handleStstxAmountChange(ststxBalance)} customStyle="w-12 h-12" iconName="PlusCircle" />
                        </View>
                    </View>
                    ...
                    <View className="mt-6">
                        {isSubmitting ? (
                            ...
                            <Button text="Add Liquidity" onPress={handleAddLiquidity} accent iconName="DatabaseIcon" />
                        )}
                    </View>
                </View>
            ) : (
                <View className="space-y-4 mt-4">
                    <View>
                        ...
                        <View className="flex-row items-center space-x-2">
                            ...
                            />
                            <Button text="MAX" onPress={() => setLpAmount(lpBalance)} customStyle="w-12 h-12" iconName="PlusCircle" />
                        </View>
                        <TextWithFont customStyle="text-gray-400 text-[10px] mt-2">
                            You will receive ~{(Number(lpAmount || 0) / 2).toFixed(4)} STX and ~{(Number(lpAmount || 0) / 2).toFixed(4)} stSTX
                        </TextWithFont>
                    </View>
                    ...
                    <View className="mt-6">
                        {isSubmitting ? (
                            ...
                            <Button text="Withdraw Liquidity" onPress={handleWithdrawLiquidity} accent iconName="RefreshCw" />
                        )}
                    </View>
                </View>
            )}
